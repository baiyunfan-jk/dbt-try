...
with spend as (select * from {{ref('rta_cost_media_bckt_accgroup_request')}}),
  request as (select * from {{ref('rta_req_media_bckt_accgroup_request')}}),
  funnel as (select * from {{ref('funnel_ad_fnnl_1_base')}}),
incremental_table_data as (select
a.pday,
a.media,
a.bucket,
a.config_id,
...
from
(
select pday ,media,bucket,config_id,strategys_name from funnel group by 1,2,3,4,5
union
select pday ,media,bucket,config_id,strategys_name from spend group by 1,2,3,4,5
union
select pday ,media,bucket,config_id,strategys_name from request where config_id not in ('','无') and config_id is not null
group by 1,2,3,4,5
)a
