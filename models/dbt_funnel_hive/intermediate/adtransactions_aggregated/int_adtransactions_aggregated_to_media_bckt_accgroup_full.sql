...
with front as (select * from {{ref('_adtransactions_aggregated_to_media_bckt_accgroup_front')}}),
back as (select * from {{ref('_adtransactions_aggregated_to_media_bckt_accgroup_back')}}),
stgy as (select * from {{ref('stg_glaucus__rta_bckt_accgroup_config')}}),
...
left join stgy 
on 
  front.win_config_id =  stgy.config_id and front.req_bucket =  stgy.bucket
